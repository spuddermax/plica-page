...
void printHelp()
{
    ...
    out << "Usage: plicapage [options] [files...]" << endl;
    out << endl;

    out << "PlicaPage provides a virtual printer for CUPS. This can be used" << endl;
...
{
    QTextStream out(stdout);
    out << "plicapage " << FULL_VERSION << endl;
}
...
void readEnvFile()
{
    ...
    QFile envFile(cacheDir + "/plicapage.env");
    if (envFile.exists())
    {
...
int main(int argc, char *argv[])
{
    ...
    if (args.startedFromCups)
    {
        ...
        {
            Log::info("Start plicapage '%s'", file.c_str());
            PlicaPageDbus::runPlicaPage(QString::fromStdString(file));
        }
        return 0;
    }
    ...
    translator.load(QString("%1/plicapage_%2.qm").arg(TRANSLATIONS_DIR, QLocale::system().name()));
    application.installTranslator(&translator);


    PlicaPageDbus dbus(DBUS_SERVICE_NAME, DBUS_OBJECT_PATH);
