...
void BooFile::read()
{
    ...
    while (!file.atEnd())
    {
        ...
        if (line.startsWith("@PJL"))
        {
            ...
            // PlicaPage commands ................................
            // "BOOMAGA" is accepted so projects saved by Boomaga still load.
            if (command == "PLICAPAGE" || command == "BOOMAGA")
            {
...
static void writeCommand(QFile *out, const QString &command, const QString &data)
{
    ...
    write(out, QString("@PJL PLICAPAGE %1=\"%2\"\n").arg(command).arg(v).toLocal8Bit());
}
...
void BooFile::save(const QString &fileName)
{
    ...
    write(&file, "\x1B%-12345X@PJL PLICAPAGE_PROJECT\n");
