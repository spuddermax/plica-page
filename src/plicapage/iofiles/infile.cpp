...
InFile::Type InFile::getType(const QString &fileName)
{
    ...
    QByteArray mark = file.read(32);
    file.close();

    // Read the CUPS spool magic .................
    // Boomaga's spelling is accepted so a job spooled by it can still be
    // opened here; PlicaPage's own backend always writes the new one.
    if (mark.startsWith("\033CUPS_PLICAPAGE") ||
        mark.startsWith("\033CUPS_BOOMAGA"))     // <- Boomaga compatibility
        return Type::CupsBoo;
    ...
    // Read a saved project ......................
    if (mark.startsWith("\x1B%-12345X@PJL PLICAPAGE_PROJECT") ||
        mark.startsWith("\x1B%-12345X@PJL BOOMAGA_PROJECT")  ||  // <- Boomaga
        mark.startsWith("\x1B%-12345X@PJL BOOMAGA_PROGECT"))     // <- and its typo
        return Type::Boo;
