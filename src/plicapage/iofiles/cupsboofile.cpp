...
void CupsBooFile::read()
{
    ...
    while (!file.atEnd())
    {
         ...
         // Boomaga's separator is accepted alongside our own, so a spool file
         // written by it still parses. Our backend only writes the new one.
         if (line.startsWith("CUPS_PLICAPAGE_DATA") ||
             line.startsWith("CUPS_BOOMAGA_DATA"))
             break;
    }
    ...
    {
        throw PlicaPageError(tr("I can't read file \"%1\" either because it's not a supported file type, "
